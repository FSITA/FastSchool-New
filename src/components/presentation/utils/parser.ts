...
export class SlideParser {
  ...
  public parseChunk(chunk: string): PlateSlide[] {
    ...
    // Always append new content to buffer instead of replacing
    // This prevents slides from disappearing during generation
    this.buffer = chunk;
  ...
  private processSections(): PlateSlide[] {
    ...
    // Only add slides that don't already exist (prevent duplicates)
    const existingSlideIds = new Set(this.parsedSlides.map(slide => slide.id));
    const uniqueNewSlides = newSlides.filter(slide => !existingSlideIds.has(slide.id));
    
    this.parsedSlides = [...this.parsedSlides, ...uniqueNewSlides];
    this.completedSections = [];

    return uniqueNewSlides;
  }
  ...
  private convertSectionToPlate = (sectionString: string): PlateSlide => {
    ...
    // Generate a stable ID based on section content to prevent duplicates
    const sectionContent = sectionString.substring(0, 100); // Use first 100 chars as identifier
    let slideId = this.sectionIdMap.get(sectionContent);
    
    if (!slideId) {
      slideId = `slide-${++this.slideCounter}`;
    ...
    if (tableNode) {
      ...
      for (const trNode of trNodes) {
        ...
        for (const tdNode of tdNodes) {
          ...
          if (valueNode) {
            if (tdType === "label") {
              label = this.getTextContent(valueNode).trim();
            } else if (tdType === "data") {
              value = parseFloat(this.getTextContent(valueNode).trim()) || 0;
            }
          }
        }
