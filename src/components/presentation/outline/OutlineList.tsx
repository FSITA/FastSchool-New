  ...
  const handleAddCard = () => {
    const newId =
      items.length > 0
        ...
        : "1";
    const newItems = [...items, { id: newId, title: "Nuova Struttura" }];
    setItems(newItems);
    // Update the outline in the store
    setOutline(newItems.map((item) => item.title));
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm text-foreground">Struttura</h2>
        {isGeneratingOutline && (
      ...
      <div className="flex justify-between text-sm text-muted-foreground">
        <span>{items.length} strutture totali</span>
        <span>
          {items.reduce((acc, item) => acc + item.title.length, 0)}/20000
        </span>
      </div>
      
      {/* Status message for slide count matching */}
      {items.length > 0 && items.length !== numSlides && (
        <div className="mt-2 text-center">
          <span className="text-xs text-orange-600">
            {items.length < numSlides 
              ? `Aggiungi ${numSlides - items.length} strutture in più per ${numSlides} slide`
              : `Rimuovi ${items.length - numSlides} strutture o cambia il numero di slide a ${items.length}`
            }
          </span>
        </div>
      )}
      ...
      {items.length > 0 && items.length === numSlides && (
        <div className="mt-2 text-center">
          <span className="text-xs text-green-600">
            ✓ Numero di strutture corrisponde al numero di slide selezionato
          </span>
        </div>
      )}
    </div>
