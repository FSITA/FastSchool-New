import DiagramGeneratorUniversalFormContainer from '@/components/diagram-generator/diagram-generator-universal-form-container'
import Image from 'next/image'
import Link from 'next/link'
import { ArrowLeft } from 'lucide-react'

export default function DiagramTestPage() {
  return (
    <div className="min-h-screen bg-white">
      {/* Header with Logo and Home Button */}
      <header className="relative max-w-4xl mx-auto px-6 pt-8 pb-8">
        <div className="flex flex-col items-center relative">
          {/* Home Button */}
          <Link 
            href="/"
            className="absolute top-0 left-0 flex items-center gap-2 px-4 py-2 border border-gray-200 bg-white hover:bg-gray-50 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md text-[#1A1A1A]"
          >
            <ArrowLeft className="w-4 h-4" />
            Casa
          </Link>
          
          {/* Logo */}
          <Link href="/" className="inline-block mb-6">
            <Image
              src="/fastschool logo on white.png"
              alt="FastSchool Logo"
              width={300}
              height={90}
              className="h-16 w-auto opacity-90"
            />
          </Link>

          {/* Two-line Heading */}
          <h1 className="text-4xl font-bold text-[#1A1A1A] mb-3 tracking-tight text-center">
            Generatore di Diagrammi IA (Test)
          </h1>
          <p className="text-lg text-[#5A5A5A] font-normal text-center max-w-2xl">
            Test della nuova funzionalità di mappe mentali - Crea mappe mentali professionali dai tuoi testi grazie alla potenza dell'intelligenza artificiale
          </p>
        </div>
      </header>

      {/* Main Content - White rounded rectangle with form */}
      <main className="max-w-4xl mx-auto px-6 pb-12">
        <DiagramGeneratorUniversalFormContainer />
      </main>
    </div>
  )
}

