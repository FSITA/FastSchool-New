...
export default function PresentationGenerateWithIdPage() {
  ...
  const handleGenerate = () => {
    // Validate that outline count matches slide count
    if (outline.length !== numSlides) {
      // Show validation message - we'll add this as a toast or alert
      const message = outline.length < numSlides 
        ? `Hai ${outline.length} strutture generate ma hai selezionato ${numSlides} slide. Aggiungi ${numSlides - outline.length} strutture in più per continuare.`
        : `Hai ${outline.length} strutture generate ma hai selezionato ${numSlides} slide. Rimuovi ${outline.length - numSlides} strutture o cambia il numero di slide.`;
      
      // For now, we'll use alert - you can replace this with a toast notification
      alert(message);
      return;
    }
  ...
  return (
    <ThemeBackground>
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t bg-background/80 p-4 backdrop-blur-sm">
        {/* Validation Messages */}
        {outline.length > 0 && outline.length !== numSlides && (
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-orange-200 bg-orange-50 p-3 text-orange-800">
            <AlertCircle className="h-4 w-4" />
            <div className="text-sm">
              {outline.length < numSlides ? (
                <>
                  Hai <strong>{outline.length}</strong> strutture generate ma hai selezionato <strong>{numSlides}</strong> slide. 
                  Aggiungi <strong>{numSlides - outline.length}</strong> strutture in più per continuare.
                </>
              ) : (
                <>
                  Hai <strong>{outline.length}</strong> strutture generate ma hai selezionato <strong>{numSlides}</strong> slide. 
                  Rimuovi <strong>{outline.length - numSlides}</strong> strutture o cambia il numero di slide.
                </>
              )}
            </div>
          </div>
        ...
        {isFromUniversalForm && originalLanguage && language !== originalLanguage && (
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-red-800">
            <AlertCircle className="h-4 w-4" />
            <div className="text-sm">
              <strong>Discrepanza Linguistica:</strong> Hai selezionato <strong>{language}</strong> per la generazione delle slide, 
              ma <strong>{originalLanguage}</strong> è stato utilizzato per la generazione della struttura. Questo potrebbe causare inconsistenze. 
              Seleziona la stessa lingua o rigenera la struttura.
            </div>
          </div>
        )}
        ...
        <div className="flex justify-center">
          ...
          >
            ...
            {isGeneratingPresentation ? "Generazione..." : "Genera Presentazione"}
          </Button>
        </div>
      </div>
    </ThemeBackground>
